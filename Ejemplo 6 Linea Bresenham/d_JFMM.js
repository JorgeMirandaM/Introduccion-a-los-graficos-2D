...
var segundoPunto=true;  //bandera para controlar los clics
...
function dibujar(event){  //Esta funci�n se ejecuta cada que se hace clic sobre el lienzo
  ...
  if (primerPunto) {  //Si es el primer clic, se lee el primer punto de la l�nea
    ...
    ponerPixel(contexto, xi, yi, c);  //ponemos el pixel en el color seleccionado
    contexto.fillText("(" + xi + "," + yi + ")", xi + 4, yi); //mostramos la coordenada
    primerPunto = !primerPunto;
  }
  else if(segundoPunto) {//Si es el segundo clic, pintamos la l�nea con los valores xi, yi
...
function lineaBresenham(x0, y0, x1, y1, contexto, color){
   ...
   contexto.fillText( "(" + x0 + "," + y0 + ")", x0+4, y0); //mostramos la coordenada
}
